...
;(function (global, factory) {
	...
	Page.prototype.setPage = function (page) {
		...
		return new Promise(function (resolve, reject) {
			if (page > 0 && page <= _this18.max) {
				_this18.page = page;
				_this18.trigger().then(function () {
					resolve();
				}).catch(function () {
					reject();
				});
			} else {
				console.warn("Pagination Error - Requested page is out of range of 1 - " + _this18.max + ":", page);
				reject();
			}
		});
	};
	...
	Page.prototype.nextPage = function () {
		...
		return new Promise(function (resolve, reject) {
			if (_this20.page < _this20.max) {
				...
				if (!_this20.progressiveLoad) {
					console.warn("Pagination Error - Next page would be greater than maximum page of " + _this20.max + ":", _this20.max + 1);
				}
				reject();
			}
		});
	...
	Page.prototype.trigger = function () {
		...
		return new Promise(function (resolve, reject) {
			...
			switch (_this21.mode) {
				case "local":
					left = _this21.table.rowManager.scrollLeft;

					_this21.table.rowManager.refreshActiveData("page");
					_this21.table.rowManager.scrollHorizontal(left);

					_this21.table.options.pageLoaded.call(_this21.table, _this21.getPage());
					resolve();
					break;
				...
				case "progressive_scroll":
					_this21.table.modules.ajax.blockActiveRequest();
					_this21._getRemotePage().then(function () {
						resolve();
					}).catch(function () {
						reject();
					});
				...
				default:
					console.warn("Pagination Error - no such pagination mode:", _this21.mode);
					reject();
			}
		});
	};
	...
	Page.prototype._getRemotePage = function () {
		...
		return new Promise(function (resolve, reject) {
			...
			pageParams[_this22.paginationDataSentNames.page] = self.page;

			//set page size if defined
			if (_this22.size) {
				pageParams[_this22.paginationDataSentNames.size] = _this22.size;
			}

			//set sort data if defined
			if (_this22.table.options.ajaxSorting && _this22.table.modExists("sort")) {
				var sorters = self.table.modules.sort.getSort();
				...
				pageParams[_this22.paginationDataSentNames.sorters] = sorters;
			}

			//set filter data if defined
			if (_this22.table.options.ajaxFiltering && _this22.table.modExists("filter")) {
				var filters = self.table.modules.filter.getFilters(true, true);
				pageParams[_this22.paginationDataSentNames.filters] = filters;
			}

			self.table.modules.ajax.setParams(pageParams);

			self.table.modules.ajax.sendRequest(_this22.progressiveLoad).then(function (data) {
