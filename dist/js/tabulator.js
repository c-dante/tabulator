...
;(function (global, factory) {
	...
	DataTree.prototype.generateChildren = function (row) {
		...
		row.getData()[this.field].forEach(function (childData) {
			...
			childRow.modules.dataTree.open = _this30.startOpen(row, childRow.modules.dataTree.index);
	...
	Group.prototype.createValueGroups = function () {
		...
		if (this.groupManager.allowedValues && this.groupManager.allowedValues[level]) {
			this.groupManager.allowedValues[level].forEach(function (value) {
				_this34._createGroup(value, level);
			});
		}
	};
	...
	Page.prototype.generatePageSizeSelectList = function () {
		...
		if (this.pageSizeSelect) {
			...
			while (this.pageSizeSelect.firstChild) {
				...
				_this36.pageSizeSelect.appendChild(itemEl);
			});
	...
	Page.prototype.setPage = function (page) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (page > 0 && page <= _this37.max) {
				...
				console.warn("Pagination Error - Requested page is out of range of 1 - " + _this37.max + ":", page);
				reject();
			}
		});
	};
	...
	Page.prototype.setPageToRow = function (row) {
		...
		return new Promise(function (resolve, reject) {
			...
			var rows = _this38.table.rowManager.getDisplayRows(_this38.displayIndex - 1);
			var index = rows.indexOf(row);

			if (index > -1) {
				var page = Math.ceil((index + 1) / _this38.size);

				_this38.setPage(page).then(function () {
					resolve();
				}).catch(function () {
					reject();
				});
	...
	Page.prototype.nextPage = function () {
		...
		return new Promise(function (resolve, reject) {
			if (_this40.page < _this40.max) {
				...
				if (!_this40.progressiveLoad) {
					console.warn("Pagination Error - Next page would be greater than maximum page of " + _this40.max + ":", _this40.max + 1);
				}
				reject();
			}
		});
	...
	Page.prototype.trigger = function () {
		...
		return new Promise(function (resolve, reject) {
			...
			switch (_this41.mode) {
				case "local":
					left = _this41.table.rowManager.scrollLeft;

					_this41.table.rowManager.refreshActiveData("page");
					_this41.table.rowManager.scrollHorizontal(left);

					_this41.table.options.pageLoaded.call(_this41.table, _this41.getPage());
					resolve();
					break;
				...
				case "progressive_scroll":
					_this41.table.modules.ajax.blockActiveRequest();
					_this41._getRemotePage().then(function () {
						resolve();
					}).catch(function () {
						reject();
					});
				...
				default:
					console.warn("Pagination Error - no such pagination mode:", _this41.mode);
					reject();
			}
		});
	};
	...
	Page.prototype._getRemotePage = function () {
		...
		return new Promise(function (resolve, reject) {
			...
			pageParams[_this42.paginationDataSentNames.page] = self.page;

			//set page size if defined
			if (_this42.size) {
				pageParams[_this42.paginationDataSentNames.size] = _this42.size;
			}

			//set sort data if defined
			if (_this42.table.options.ajaxSorting && _this42.table.modExists("sort")) {
				var sorters = self.table.modules.sort.getSort();
				...
				pageParams[_this42.paginationDataSentNames.sorters] = sorters;
			}

			//set filter data if defined
			if (_this42.table.options.ajaxFiltering && _this42.table.modExists("filter")) {
				var filters = self.table.modules.filter.getFilters(true, true);
				pageParams[_this42.paginationDataSentNames.filters] = filters;
			}

			self.table.modules.ajax.setParams(pageParams);

			self.table.modules.ajax.sendRequest(_this42.progressiveLoad).then(function (data) {
